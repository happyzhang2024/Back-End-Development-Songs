...
def songs():
    ...
    return parse_json({"songs": all_songs}), 200
...
def create_song():
    ...
    if "id" not in song:
        return {"message": "song must include id"}, 400

    try:
        song_id = int(song["id"])
    except ValueError:
        return {"message": "id must be an integer"}, 400
    ...
    song["_id"] = str(inserted.inserted_id)
...
def update_song(id):
    ...
    existing = db.songs.find_one({"id": id})
    if not existing:
